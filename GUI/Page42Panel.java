...
public class Page42Panel extends JPanel {
    ...
    public Page42Panel(CinemaApp app, String setName, String setPrice, String setImage) {
        ...
        int totalPrice = session.getTotalPrice() + Integer.parseInt(setPrice.replace(" THB", ""));
        JLabel priceLabel = new JLabel(totalPrice + " THB");
...
centerPanel.setBorder(BorderFactory.createEmptyBorder(30, 30, 30, 30)); // padding รอบนอก

// เติมกล่องว่างเพื่อดันไปกลาง
centerPanel.add(Box.createHorizontalGlue());

// เพิ่มที่ว่างด้านซ้ายของรูปหนัง (เช่น 60px)
...
// ช่องว่างระหว่างรูปหนังกับ Add-on
centerPanel.add(Box.createRigidArea(new Dimension(40, 0))); // เพิ่มเป็น 40px
...
// ช่องว่างระหว่าง Add-on กับข้อความ
centerPanel.add(Box.createRigidArea(new Dimension(60, 0))); // เพิ่มเป็น 60px
...
// เติมกล่องว่างอีกด้านเพื่อบาลานซ์
centerPanel.add(Box.createHorizontalGlue());
...
add(centerPanel, BorderLayout.CENTER);
    ...
    // เก็บค่า Add-on ลงใน session ก่อน
    session.setSelectedAddonName(setName);
    session.setSelectedAddonPrice(setPrice);
    session.setSelectedAddonImage(setImage);
    session.setTotalPrice(session.getTotalPrice() + Integer.parseInt(setPrice.replace(" THB", "")));

    // ไปหน้า Page5 โดยบอกว่า มาจาก Page42Panel
